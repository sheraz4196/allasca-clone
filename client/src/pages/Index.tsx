...
const Index = () => {
  ...
  useEffect(() => {
    ...
    // Set up event listeners for action buttons to scroll to contact
    const setupActionButtons = () => {
      // Find buttons with specific text content that should scroll to contact
      ...
      document.querySelectorAll('button').forEach(button => {
        ...
        // Skip buttons in the navbar to avoid interfering with their functionality
        const isNavbarButton = button.closest('header') || button.closest('nav');
        // Skip form submit buttons to avoid interfering with form validation
        ...
        if (buttonText && actionButtonTexts.includes(buttonText) && !isNavbarButton && !isFormSubmitButton) {
          ...
          button.removeEventListener('click', scrollToContactHandler);
          button.addEventListener('click', scrollToContactHandler);
        }
      });
    };
    ...
    // Run the setup after a short delay to ensure DOM is fully loaded
    setTimeout(setupActionButtons, 500);

    // Throttle the MutationObserver callback for better performance
    let timeoutId: NodeJS.Timeout;
    const observer = new MutationObserver(() => {
      clearTimeout(timeoutId);
      timeoutId = setTimeout(setupActionButtons, 300);
    });

    // Only observe specific containers instead of entire body for better performance
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-purple-50">
      ...
      <main>
        ...
        {/* See Our Work in Action Video Section */}
        <section className="py-16 bg-white">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-6xl mx-auto">
              <div className="rounded-xl overflow-hidden shadow-lg" style={{ aspectRatio: '9/16' }}>
                <iframe
                  width="100%"
                  height="100%"
          ...
          <div className="container max-w-6xl mx-auto px-4">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[0] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[0] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  The duration depends on the project size and scope. A full home renovation typically takes 3 to 6 months, while smaller remodels like kitchens or bathrooms can take 6 to 10 weeks. We provide a clear timeline before starting and update you regularly throughout the process.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[1] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[1] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Costs vary widely based on materials, finishes, and square footage. On average, major home renovations in Toronto range from $150 to $350 per square foot. We provide a detailed, transparent quote after the initial design and consultation.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[2] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[2] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Yes — if your project involves structural changes, wall removal, additions, or new plumbing/electrical systems, a permit is required. We handle all permit drawings and submissions directly with the City of Toronto to save you time.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[3] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[3] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  A renovation updates existing finishes or layouts without major changes, while a remodeling project can include structural changes, extensions, or complete redesigns. We offer both depending on your goals.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[4] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[4] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Absolutely. We're a full-service design-build company, meaning we take care of both the creative design and the actual construction. This ensures faster timelines, fewer miscommunications, and a smoother process from start to finish.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[5] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[5] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  No problem — we can work with your existing plans and help refine them to meet building code and zoning requirements. We also provide value-engineering suggestions to optimize design and cost.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[6] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[6] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Yes. All our projects include a comprehensive workmanship warranty. We also offer post-construction check-ins to ensure you're fully satisfied with every detail even after completion.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[7] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[7] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Yes, our portfolio includes modern, luxury, and custom-built homes across the GTA. We collaborate closely with clients to select premium finishes, fixtures, and architectural details that elevate every project.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[8] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[8] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  It depends on the project scope. For smaller renovations, yes — but for major structural or full-home projects, we recommend temporary relocation for safety and efficiency. We help plan logistics to minimize disruption.
                </p>
              )}
              ...
              >
                ...
                <ChevronDown className={`h-6 w-6 text-casa-purple flex-shrink-0 transition-transform ml-4 ${expandedQuestions[9] ? 'rotate-180' : ''}`} />
              </button>
              {expandedQuestions[9] && (
                <p className="text-gray-700 leading-relaxed px-6 pb-4">
                  Simply book a free consultation with our design-build experts. We'll review your goals, property, and budget, then prepare a proposal outlining design ideas, estimated cost, and next steps.
                </p>
              )}
            </div>
          </div>
