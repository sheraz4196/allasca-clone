...
const ServicesGrid = () => {
  ...
  return (
    <section id="our-services" className="py-20 bg-gradient-to-br from-purple-50 to-white">
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-poppins font-bold text-casa-navy mb-4">We Offer</h2>
          <p className="text-lg text-casa-gray max-w-3xl mx-auto">
            Comprehensive construction and renovation services across the Greater Toronto Area, delivering quality craftsmanship in every project.
          </p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6 lg:gap-8">
          {servicesItems.map((item, index) => (
            <ServiceCard key={`${item.title}-${index}`} item={item} index={index} onGetQuote={scrollToContact} />
          ))}
        </div>
      </div>
    </section>
...
const ServiceCard = memo(({ item, index, onGetQuote }: { item: any, index: number, onGetQuote: () => void }) => {
  return (
