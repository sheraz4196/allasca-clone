...
const ImageCarousel = ({ images, variant = 'default', prioritizeFirst = false }: ImageCarouselProps) => {
  ...
  return (
    <div className={`w-full ${isSection ? 'max-w-none' : 'max-w-7xl mx-auto'} ${isSection ? '' : 'px-4'}`}>
      <Carousel 
        className="w-full"
        opts={{
          align: "start",
      ...
      >
          ...
          {images.map((image, index) => (
            ...
            >
              <div className="h-full">
                <div className={`${isSection ? 'aspect-[4/3]' : 'aspect-[4/3]'} w-full overflow-hidden rounded-lg shadow-lg bg-gray-100`}>
                  <img
                    src={image.src}
                    alt={image.alt}
                    loading={prioritizeFirst && index === 0 ? "eager" : "lazy"}
        ...
        <div className={`flex justify-center items-center ${isSection ? 'mt-4' : 'mt-6'} gap-4`}>
